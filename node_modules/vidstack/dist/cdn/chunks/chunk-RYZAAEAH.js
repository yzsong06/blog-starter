...
var TextTrack = class extends EventsTarget {
  constructor(init) {
    ...
    for (const prop of Object.keys(init))
      this[prop] = init[prop];
    if (init.content) {
      import('./dev-GQQUQYRY.js').then(({ parseText, VTTCue, VTTRegion }) => {
        if (init.type === "json") {
  ...
  async _load() {
    ...
    try {
      const { parseResponse, VTTCue, VTTRegion } = await import('./dev-GQQUQYRY.js'), crossorigin = this[TEXT_TRACK_CROSSORIGIN]?.();
