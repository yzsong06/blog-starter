...
var Time = class extends Component {
  onAttach() {
    this.a = useMedia();
    this.ve = computed(this.Cc.bind(this));
  }
  Cc() {
    const { type, remainder, padHours, padMinutes, showHours } = this.$props, seconds = this.Zh(type()), duration = this.a.$store.duration();
  ...
  Zh(type) {
    const { bufferedEnd, duration, currentTime } = this.a.$store;
    switch (type) {
      case "buffered":
        return bufferedEnd();
      case "duration":
  ...
  render() {
    return (() => {
      ...
      $$_insert_at_marker_lite($$_expr, this.ve);
      return $$_root;
    })();
  }
};
